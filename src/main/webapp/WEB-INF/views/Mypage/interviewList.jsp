			...
			<table class="table table-hover">
				...
				<c:choose>
					...
					<td>
						...
						<input type="hid-den" id="id" value="${row.sitter_id}"/>
						<c:choose>
							<c:when test="${flag eq 'sitter' }"><input type="hid-den" id="agree_${row.idx }" value="${row.sitter_agree}" /></c:when>
							<c:otherwise><input type="hid-den" id="agree_${row.idx }" value="${row.parents_agree}" /></c:otherwise>
						</c:choose>
					</td>
					<td>
						<c:choose>
							<c:when test="${flag eq 'sitter' }">
								<c:choose>
									...
									<c:otherwise>
										<button type="button" class="btn btn-warning" onClick="parentsView(${row.request_idx});">자세히</button>
									</c:otherwise>
								</c:choose>
							</c:when>
							<c:otherwise> 
								<c:choose>
									<c:when test="${row.request_idx eq 0 }">
										<button type="button" class="btn btn-warning" onClick="sitterView('${row.sitter_id}')">자세히</button>
									</c:when>
									<c:otherwise>
										<button type="button" class="btn btn-sm btn-basic">받은 요청</button>
									</c:otherwise>
					...
					<c:choose>
						...
						<c:choose>
							...
							<c:otherwise>
								<td><button type="button" class="btn btn-dark " id="accept_${row.idx }" onClick="javascript:acceptThis(${row.idx});">수락</button></td>	
							</c:otherwise>
							</c:choose>
								<td><button type="button" class="btn btn-dark" id="delete" onClick="javascript:deleteThis(${row.idx});">취소</button></td>
							</c:when>
						<c:otherwise>
							<td><button type="button" class="btn btn-info" id="diary" onClick="javascript:openCalendar(${row.idx});">알림장</button></td>
							<td><button type="button" class="btn btn-success" onClick="javacript:openComment(${row.idx});">후기</button></td>
						</c:otherwise>
					</c:choose>
				</tr>
				</c:forEach>
