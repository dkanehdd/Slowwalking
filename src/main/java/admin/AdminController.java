...
public class AdminController {
	...
	public String modify(Model model, HttpServletRequest req, HttpSession session) {
		...
		MemberDTO dto = sqlSession.getMapper(AdminMemberImpl.class).view(memberDTO);
	...
	public String modifyAction(HttpSession session, MemberDTO memberDTO) {
		...
		int applyRow = sqlSession.getMapper(AdminMemberImpl.class).modify(memberDTO);
	...
	public String delete(HttpServletRequest req) {
		...
		sqlSession.getMapper(AdminMemberImpl.class).delete(req.getParameter("id"));
	...
	public String mailSending(HttpServletRequest request, ModelMap mo) throws AddressException, MessagingException {
		...
		for (int i = 0; i < tomailarr.length; i++) {
			...
			// MimeMessage 생성
			mimeMessage.setFrom(new InternetAddress("cysik92@naver.com")); // 발신자 셋팅 , 보내는 사람의 이메일주소를 한번 더 입력합니다. 이때는
																			// 이메일 풀 주소를 다 작성해주세요.
			mimeMessage.setRecipient(Message.RecipientType.TO, new InternetAddress(tomailarr[i])); // 수신자셋팅 //.TO 외에
																									// .CC(참조)
																									// .BCC(숨은참조) 도 있음
